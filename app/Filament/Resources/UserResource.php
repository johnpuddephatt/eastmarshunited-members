...
class UserResource extends Resource
{
    ...
    {
        return $form->schema([
            Components\Tabs::make("Label")->tabs([
                Components\Tab::make("Account", [
                    ...
                    Components\Select::make("type")
                        ...
                        ->default("member")
                        ->dependable(),
                    Components\Checkbox::make("approved")->when(
                        fn($record) => $record->type === "organisation"
                    ),
                    Components\Textarea::make("notes"),
                ]),
                Components\Tab::make("Timebanking", [
    ...
    public static function table(Table $table)
    {
        return $table
            ->columns([
                ...
                Columns\Text::make("date_of_birth"),
                Columns\Text::make("created_at"),
                Columns\Text::make("type"),
                Columns\Boolean::make("has_completed_profile")->label(
                    "Timebanking"
                ),
            ])
            ->filters([
                Filter::make(
                    "Only members",
                    fn($query) => $query->where("type", "member")
                ),
                Filter::make(
                    "Only supporters",
                    fn($query) => $query->where("type", "supporter")
                ),
                Filter::make(
                    "Only organisations",
                    fn($query) => $query->where("type", "organisation")
                ),
                Filter::make(
                    "Show deleted users",
